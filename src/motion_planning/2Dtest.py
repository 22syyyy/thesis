...
def optimize_path_with_casadi(path, obstacles):
    ...
    for k in range(3, N-4):                      
        ...
        for Pj in obstacles:           
            add_linear_separation(opti, Qk, Pj, margin=0.01)
    ...
    total_time = (N-3)*dt
    w1, w2, w3 = 0, 1, 1
    opti.minimize(w1 * fit_cost + w2 * jerk_cost + w3 * total_time)
...
def generate_bspline_path(q, num_points=300):
    t_vals = np.linspace(3, len(q)-1, num_points)  
...
def main(gx=15.0, gy=13.0):
    ...
    if path is None:
        ...
        q_sol, dt_sol, q0 = optimize_path_with_casadi(rrt.final_path, rrt.octagon_list)
        curve = generate_bspline_path(q_sol)
